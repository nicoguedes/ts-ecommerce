export default class CpfValidator{
    ...
    private getVerificationDigit = (cpfText) => {
    ...
    private validateVerificationDigit = (cpfText, digitResult) => {
    ...
    validate(cpfText) {
        if (!cpfText || cpfText.length < 11 || cpfText.length > 14)
            return false;
        cpfText = this.removeSpecialCharacters(cpfText);
        if (cpfText.split("").every(c => c === cpfText[0])) return false;
        try {
            const verificationDigit = this.getVerificationDigit(cpfText);
            return this.validateVerificationDigit(cpfText, verificationDigit);
        }
        catch (e) {
